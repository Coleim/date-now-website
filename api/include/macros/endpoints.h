...
#define ERROR_REPLY_500 error_reply_map(error_reply, 500, "Internal error", 500); \
	mg_http_reply(c, error_reply->code_http, JSON_HEADER, error_reply->json);
#define ERROR_REPLY_SQL error_reply_map(error_reply, query_code, "Error SQL Query.", 500); \
	mg_http_reply(c, error_reply->code_http, JSON_HEADER, error_reply->json);
#define ERROR_REPLY_405 error_reply_map(error_reply, 405, "Method not allowed", 405); \
	mg_http_reply(c, error_reply->code_http, JSON_HEADER, error_reply->json);
#define ERROR_REPLY_400(message) error_reply_map(error_reply, 400, message, 400); \
	mg_http_reply(c, error_reply->code_http, JSON_HEADER, error_reply->json);
#define ERROR_REPLY_404 error_reply_map(error_reply, 404, "Not found", 404); \
	mg_http_reply(c, error_reply->code_http, JSON_HEADER, error_reply->json);
