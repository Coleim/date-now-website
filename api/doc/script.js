...
window.addEventListener("load", async () => {
        ...
        appendCategories(apiContainer, tables);
...
const getUriParameterId = (parameter) => {
        ...
        return `uri-parameter-${parameter.replaceAll(regex, "")}`;
};

const getQueryParameterId = (parameter) => {
        const regex = new RegExp("{|}", "g");

        return `query-parameter-${parameter.replaceAll(regex, "")}`;
};
...
const getEndpointNode = (endpoint) => {
        ...
        if (uriParameters !== undefined && uriParameters.length > 0) {
                ...
                const uriParametersContainerTitle =
        ...
        if (queryParameters !== undefined && queryParameters.length > 0) {
                ...
                const queryParametersContainerTitle =
                        document.createElement("h4");
                queryParametersContainerTitle.innerHTML = "Query Parameters";
                queryParametersContainer.appendChild(
                        queryParametersContainerTitle,
                );
